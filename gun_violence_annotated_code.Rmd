...
# convert multiple MeSH queries into list, remove leading/trailing whitespace, convert to upper
...
mortality$Publications <- sapply(mortality$PubMed.Query, 
                                 function(x) colSums(getPubmedTrend(x, 2004, 2015))[2])
```
...
for (row in 1:nrow(mortality)) {
  ...
  # FedReporter match for "ALZHEIMER DISEASE" is "ALZHEIMER'S DISEASE"
  ...
  matchTerm <- vapply(FedReporter$Project_Terms, function(x) {
   ifelse(length(intersect(terms, unlist(x)) != 0), TRUE, FALSE)
